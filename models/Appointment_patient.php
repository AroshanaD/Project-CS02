...
class Appointment_patient extends Models{
    ...
    public function view_appointment($id){
        ...
        ON patient_appointment.doctor_appointmentId=appointment.appointment_Id JOIN patient ON patient_appointment.patient_Id= patient.id 
    ...
    public function appointmentby_category($specialization){
        ...
        if($specialization==null){
            ...
            patient.contact_no,doctor.f_name AS 'doctor_fname', doctor.l_name AS 'doctor_lname', schedule.time from patient JOIN patient_appointment ON 
            patient.id= patient_appointment.patient_Id JOIN appointment ON appointment.appointment_id = patient_appointment.doctor_appointmentId JOIN 
            doctor ON appointment.Doctor_Id= doctor.id JOIN schedule ON appointment.schedule_id=schedule.id  WHERE appointment.Availability= 1";
            $stmt = $pdo->prepare($query);
            $stmt->execute();
        }
           ...
           patient.contact_no,doctor.f_name AS 'doctor_fname', doctor.l_name AS 'doctor_lname', schedule.time from patient JOIN patient_appointment ON 
           patient.id= patient_appointment.patient_Id JOIN appointment ON appointment.appointment_id = patient_appointment.doctor_appointmentId JOIN 
           doctor ON appointment.Doctor_Id= doctor.id JOIN schedule ON appointment.schedule_id=schedule.id  WHERE doctor.specialization_id=?"; 
            $stmt = $pdo->prepare($query);
    ...
    public function search_recept($name,$date){
        ...
        patient.contact_no,doctor.f_name AS 'doctor_fname', doctor.l_name AS 'doctor_lname', schedule.time from patient JOIN patient_appointment ON 
        patient.id= patient_appointment.patient_Id JOIN appointment ON appointment.appointment_id = patient_appointment.doctor_appointmentId JOIN 
        doctor ON appointment.Doctor_Id= doctor.id JOIN schedule ON appointment.schedule_id=schedule.id WHERE appointment.Date LIKE ? OR patient.f_name LIKE ? 
        OR patient.l_name LIKE ?";

        $stmt = $pdo->prepare($query);
        $stmt->execute([$date,$name,$name]);
    ...
    public function get_doctorApoointment($id){
        ...
        patient.contact_no,doctor.f_name AS 'doctor_fname', doctor.l_name AS 'doctor_lname', schedule.time from patient JOIN patient_appointment ON 
        patient.id= patient_appointment.patient_Id JOIN appointment ON appointment.appointment_id = patient_appointment.doctor_appointmentId JOIN 
        doctor ON appointment.Doctor_Id= doctor.id JOIN schedule ON appointment.schedule_id=schedule.id WHERE appointment.Doctor_Id=?";
    ...
    public function search_doctor($id,$date){
        ...
        patient.id= patient_appointment.patient_Id JOIN appointment ON appointment.appointment_id = patient_appointment.doctor_appointmentId JOIN 
        doctor ON appointment.Doctor_Id= doctor.id JOIN schedule ON appointment.schedule_id=schedule.id WHERE 
        appointment.Date LIKE ? AND appointment.Doctor_Id=? ";
